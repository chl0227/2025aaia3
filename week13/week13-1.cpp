...
// Moodle 典籍 adventofcode.com 網址 可看到黑底、白字的題目
// 在LeetCode  的My Playground 我的遊戲場, 可以寫程式
// LeetCode 幫你把 #include  全部都加好了, 你不用寫 #include
/// 右下角 sdin 可貼上你的程式 Input
/// 在 AdentofCode.com 登入你的 GitHub 帳號後, 便可以看到你的專屬謎題Input
/// 貼到右下角 stdin 後, 在執行, 看你的答案是多少(每個人都不一樣)
int main() {
    char c; /// 字母, 對應方向 L左轉 R右轉
    int d; /// 數字, 要轉動幾格
    int now = 50; /// 一開始的密碼鎖, 指向 50
    int ans = 0; /// 轉動時,有幾次停在 0 的地方? (我們的通關密碼)
    while ( cin >> c >> d ) { /// 一直讀資料; 獨自母 讀數字
        /// if (c=='L') cout << "往左轉" << d << "格\n";
        /// if (c=='R') cout << "往右轉" << d << "格\n";
        /// 把上面兩行的中文訊息註解掉
        if (c=='L') now = now - d; /// 減掉
